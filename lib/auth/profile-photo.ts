...
/**
 * Supabase `.update().eq(...)` can return error=null with zero matching rows.
 * Treat a returned row (or explicit count > 0) as the only successful persistence signal.
 */
export function isProfileRowPersisted(result: {
  error: unknown;
  data: unknown;
  count?: number | null;
}): boolean {
  ...
  return result.data != null && (Array.isArray(result.data) ? result.data.length > 0 : true);
}
